...
void ShaderTexLight::ChangeDirectionalLighting()
{
	DirLightData = SFXManager::GetDirectionalLighting();
}
void ShaderTexLight::ChangePointLighting(int n)
{
	assert(n >= 0 && n < POINTARRAY_SIZE);
	PointLightData[n] = SFXManager::GetPointLighting(n);
}
void ShaderTexLight::ChangeSpotLighting(int n)
{
	assert(n >= 0 && n < SPOTARRAY_SIZE);
	SpotLightData[n] = SFXManager::GetSpotLighting(n);
}

void ShaderTexLight::SetTextureResourceAndSampler(Texture* tex)
{
	pTex = tex;
	pTex->SetToContext(this->GetContext());
}

void ShaderTexLight::SendCamMatrices(const Matrix& view, const Matrix& proj)
{
...
void ShaderTexLight::SetDirectionalLightParameters(const Vect& dir, const Vect& amb, const Vect& dif, const Vect& sp)
{
...
void ShaderTexLight::SetPointLightParameters(const int n, const Vect& pos, float r, const Vect& att, const Vect& amb, const Vect& dif, const Vect& sp)
{
...
void ShaderTexLight::SetSpotLightParameters(const int n, const Vect& pos, float r, const Vect& att, const Vect& dir, float spotExp, const Vect& amb, const Vect& dif, const Vect& sp)
{
...
void ShaderTexLight::SendLightParameters(const Vect& eyepos)
{
	Data_LightParams dl;
...
void ShaderTexLight::SendWorldAndMaterial(const Matrix& world, const Vect& amb, const Vect& dif, const Vect& sp)
{
...
void ShaderTexLight::SetToContext()
{
...
void ShaderTexLight::SetToContext2D()
{
	ShaderBase::SetContext();
	ShaderBase::SetToContext_VS_PS_InputLayout();
