...
Texture::Texture(LPCWSTR filepath, D3D11_FILTER filter, UINT isotropic_level, 
	bool ComputeMip, size_t miplevel, DirectX::TEX_FILTER_FLAGS filterflags)
{
	this->Initialize(filepath, filter, isotropic_level,
		ComputeMip, miplevel, filterflags);
}

Texture::Texture(const char* filepath, D3D11_FILTER filter, UINT isotropic_level,
	bool ComputeMip, size_t miplevel, DirectX::TEX_FILTER_FLAGS filterflags)
{
	...
	mbstowcs_s(&convertedChars, file, newsize, filepath, _TRUNCATE);
	// initialize
	this->Initialize(file, filter, isotropic_level,
		ComputeMip, miplevel, filterflags);
}

void Texture::Initialize(LPCWSTR filepath, D3D11_FILTER filter, UINT isotropic_level, 
	bool ComputeMip, size_t miplevel, DirectX::TEX_FILTER_FLAGS filterflags )
{
	ID3D11Device* d3dDev = EngineAttorneyToGraphics::GetDevice();
