	...
	: camType(type)
{
	...
	if (type == Camera::Type::PERSPECTIVE_3D)
		cmd = new CameraPerspCmd(this);
	else
		cmd = new CameraOrthoCmd(this);
}
...
void Camera::setOrthographic(const float top, const float bot, const float left, const float right, const float NearDist, const float FarDist)
{
	this->topClip = top;
	this->botClip = bot;
	this->leftClip = left;
...
void Camera::privUpdatePerspective(void)
{
...
void Camera::privUpdateOrthographic(void)
{
	this->projMatrix[m0] = 2.0f / this->rightClip - this->leftClip;
	this->projMatrix[m1] = 0.0f;
	this->projMatrix[m2] = 0.0f;
	this->projMatrix[m3] = 0.0f;

	this->projMatrix[m4] = 0.0f;
	this->projMatrix[m5] = 2.0f / this->topClip - this->botClip;
	this->projMatrix[m6] = 0.0f;
	this->projMatrix[m7] = 0.0f;

	this->projMatrix[m8] = 0.0f;
	this->projMatrix[m9] = 0.0f;
	this->projMatrix[m10] = -2 / (this->farDist - this->nearDist);
	this->projMatrix[m11] = -1.0f;

	this->projMatrix[m12] = - (this->rightClip + this->leftClip) / (this->rightClip - this->leftClip);
	this->projMatrix[m13] = - (this->topClip + this->botClip) / (this->topClip - this->botClip);
	this->projMatrix[m14] = - (this->farDist + this->nearDist) / (this->farDist - this->nearDist);
	this->projMatrix[m15] = 1;
