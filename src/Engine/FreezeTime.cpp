...
#include "APlusPlusAttorneyToFreezeTime.h"
...
//		glfwPollEvents();  // We must get GLFW to rescan the keyboard
	...
	return APlusPlusAttorneyToFreezeTime::GetTime() - activetime;
}
...
bool FreezeTime::HackedKeyRelease(APPE_KEY k)
{
	if (Keyboard::IsKeyActive(k)) // Is the key pressed?
	{		
		while (Keyboard::IsKeyActive(k)) // then poll keyboard until k is released
		{
