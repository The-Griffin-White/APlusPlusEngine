...
public:
	...
	/// \brief Collision with terrain, defined by user.
	/// \ingroup COLLISION
	///
	/// \par EXAMPLE
	/// \code
	///		void ExampleGameObject::TerrainCollision()
	///		{
	///			// Where Explodes() is a user defined class
	///			this->Explodes();
	///		}
	...
	const CollisionVolumeBSphere* GetCollisionBSphere() const;
...
protected:
	...
	/// \param type The type of collision model you want to use - BSphere, AABB, OBB...
	/// \par EXAMPLE:
	/// \code
	...
	///		}
	/// \endcode
	void SetColliderModel(Model* mod, CollisionModels type);
